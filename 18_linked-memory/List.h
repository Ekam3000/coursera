...
class List {
  public:
    const T & operator[](unsigned index); // this operator is used for accessing the list L[0],L[1],L[2]...
    void insertAtFront(const T & data);

    // We define this constructor to make sure that head_ is null-initialized.
    List() : head_(nullptr) { } // initialization is done here 
    ...
    ~List() {
      ...
      ListNode *thru = head_;//This is the thing that's going to allow us to advance through the linked list starting at the head.
      while (thru != nullptr) {
  ...
  private:
    class ListNode {
      public:
        const T & data;
        ListNode *next;
        ListNode(const T & data) : data(data), next(nullptr) { }//initialization is done here 
    };
