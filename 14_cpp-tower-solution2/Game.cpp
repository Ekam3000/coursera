...
void Game::_move(
  unsigned start, unsigned end, // start is 0 and end is 3
  ...
  if (start == end) {
    ...
    _move(start + 1, end  , source, spare , target, depth); // firstly move index 1 to 4 from source to spare
    _move(start    , start, source, target, spare , depth); // index 0  from source to target 
    _move(start + 1, end  , spare , target, source, depth); // index 1 to 4 from spare to target
  }
}
...
void Game::solve() {
  _move(
    ...
    stacks_[0], //< Source stack is [0] .. here the addr of stack[0] is passed 
